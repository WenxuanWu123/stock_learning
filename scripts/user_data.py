...
def export():
    ...
    def q(sql):
        try:
            ...
            return []  # 旧库缺表（如新增的用户表）时跳过，不阻塞 rebuild

    answers = q("SELECT c.slug chapter_slug, q.q qtext, a.chosen, a.correct, a.ts"
                " FROM answers a JOIN questions q ON q.id=a.question_id"
                " JOIN chapters c ON c.id=q.chapter_id")
    wrong = q("SELECT c.slug chapter_slug, q.q qtext, w.wrong_count, w.last_ts"
              " FROM wrongbook w JOIN questions q ON q.id=w.question_id"
              " JOIN chapters c ON c.id=q.chapter_id")
    attempts = q("SELECT l.slug level_slug, a.chosen, a.score, a.ts"
                 " FROM level_attempts a JOIN levels l ON l.id=a.level_id")
    reads = q("SELECT c.slug chapter_slug, r.ts FROM lesson_reads r"
    ...
    BACKUP.write_text(json.dumps({
        ...
        "lesson_reads": [dict(r) for r in reads],
    }, ensure_ascii=False, indent=1), encoding="utf-8")
    print(f"已备份: {len(answers)} 答题 / {len(wrong)} 错题 / {len(attempts)} 关卡记录 / {len(reads)} 阅读")
...
def import_():
    ...
    for t in data.get("level_attempts", []):
        ...
        if i:
            ...
            dl += 1
    cid = {r["slug"]: r["id"] for r in conn.execute("SELECT id, slug FROM chapters")}
    nr = dr = 0
    for r in data.get("lesson_reads", []):
        i = cid.get(r["chapter_slug"])
        if i:
            conn.execute("INSERT INTO lesson_reads (chapter_id, ts) VALUES (?,?)",
                         (i, r["ts"]))
            nr += 1
        else:
            dr += 1
    conn.commit()
    conn.close()
    msg = f"已恢复: {na} 答题 / {nw} 错题 / {nl} 关卡记录 / {nr} 阅读"
    if da + dw + dl + dr:
        msg += f"；因内容已变更丢弃: 答题{da} / 错题{dw} / 关卡{dl} / 阅读{dr}"
    print(msg)
